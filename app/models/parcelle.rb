...
# ----- Finders -----
  ...
  def self.find_by_saison(*args)
    with_scope(:find =>
                {:conditions => ["saison_id = ?", session[:current_saison_id]], :order => :typeculture_id }) do
        find(*args)
      end
  end
  ...
  def self.find_by_zone(zone_id)
    ...
    zonetopas.each do |zonetopa|
      if zonetopa.parcelle.saison_id == session[:current_saison_id]
  ...
  def self.get_parcelles_from_culture(culture)
    parcelles = culture.parcelles.select {|p| p.saison_id.eql?(session[:current_saison_id]) }
  end
